...
class GraniteMoeHybridBlock(nn.Module):
    def __init__(self, config: GraniteMoeHybridConfig, rotary_emb: RotaryEmbedding):
        super(GraniteMoeHybridBlock, self).__init__()
        self.config = config
...
class GraniteMoeHybridHeadless(nn.Module):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if config is not None:
            ...
            self.config = GraniteMoeHybridConfig()
...
class GraniteMoeHybrid(nn.Module):
    ...
    @classmethod
    def from_config(cls, config: GraniteMoeHybridConfig) -> "GraniteMoeHybrid":
        return cls(config)

    def get_config(self) -> GraniteMoeHybridConfig:
        return self.config
...
def _weight_fusion(
    input_sd: Mapping, model_config: Optional[GraniteMoeHybridConfig] = None, **kwargs
):
...
def _hf_to_fms_rope(
    ...
    model_config: Optional[GraniteMoeHybridConfig] = None,
    **kwargs,
) -> Mapping[str, Any]:
    new_sd = {}
...
serialization.register_adapter_step(
    _architecture_name, "hf_gptq_fusion_check", _hf_gptq_granite_v4_check
)
