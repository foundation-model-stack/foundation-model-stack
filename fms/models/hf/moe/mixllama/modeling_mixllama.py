...
class MixLlamaAttention(LlamaFlashAttention2):
    ...
    def __init__(self, config: MixLlamaConfig, layer_idx:int):
        ...
        self.router = nn.Linear(self.hidden_size, self.num_experts, bias=False)
        delattr(self, "q_proj")
        self.q_proj = nn.ModuleList([nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias) for _ in range(self.num_experts)])
    ...
    def forward(
        ...
        **kwargs,
        ...
        # router_logits: (batch * sequence_length, n_experts)
        router_logits = self.router(hidden_states.view(-1, hidden_dim))
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)
        routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        routing_weights = routing_weights.to(hidden_states.dtype)
        ...
        query_states = torch.zeros(
            (bsz * q_len, hidden_dim), dtype=hidden_states.dtype, device=hidden_states.device
        )

        for expert_idx in range(self.num_experts):
            expert = self.q_proj[expert_idx]
            idx, top_x = torch.where(expert_mask[expert_idx])
            ...
            current_state = hidden_states.view(-1, hidden_dim)[None, top_x_list].view(-1, hidden_dim)
            current_hidden_states = expert(current_state) * routing_weights[top_x_list, idx_list, None]
            query_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))
...
class MixLlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value  = self_attn_out[:3]
        ...
        if output_router_logits:
            if isinstance(self.self_attn, MixLlamaAttention):
                attn_router_logits = self_attn_out[-1]
                outputs += (attn_router_logits,)
            if isinstance(self.mlp, MixLlamaMLP):
                mlp_router_logits = mlp_out[-1]
