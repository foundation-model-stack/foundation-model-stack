...
class SSM(nn.Module):
    ...
    def forward(self, input_states, mask, past_key_value_state: Optional[SSMCacheUnit]=None, **kwargs):
        ...
        if past_key_value_state is not None:
            ssm_state = past_key_value_state.ssm_state.clone()
            ssm_state = ssm_state.to(hidden_states.device)
            if past_key_value_state.seqlen_offset > 0:
                conv_state = past_key_value_state.conv_state  # [batch, intermediate_size, conv_kernel_size]
        ...
        if past_key_value_state is not None and past_key_value_state.seqlen_offset > 0:
            ...
            states = (B_decay_contraction.permute(0, 1, 3, 2, 4)[..., None] * hidden_states.permute(0, 1, 3, 2, 4)[...,
                                                                              None, :]).sum(dim=3).permute(0, 1, 2, 4,
                                                                                                           3)
            if past_key_value_state is not None and past_key_value_state.seqlen_offset > 0:
                previous_states = past_key_value_state.ssm_state[:, None, ...]
            else:
