...
class ExpandableKVCacheManager(KVCacheManager):
    ...
    def __init__(self, num_layers: int, num_heads: int, emb_dim: int, tensor_parallel_size: int = 1, device: Optional[Union[str, torch.device]] = "cpu", dtype: torch.dtype = torch.float32):
        self.cache: List[Optional[Tuple[torch.Tensor, torch.Tensor]]] = [None for _ in range(num_layers)]
    ...
    def allocate_prompt_tokens(
        ...
        self.cache.clear()
        self.context_map = {i: num_tokens for i, num_tokens in enumerate(num_tokens_per_sequence)}
        ...
        for i in range(self.num_layers):
            ...
            empty_tensor_k = torch.empty(
                size=(len(num_tokens_per_sequence), self.num_heads, max_sequence_length, self.head_size),
                dtype=self.dtype,
                device=self.device
            )
        ...
        return InPlaceCacheData(data=self.cache, context_lengths=context_lengths, max_sequence_length=max_sequence_length, sequence_ids=sequence_ids, is_generating=False)


    def allocate_generated_tokens(self, sequence_ids: List[int], num_tokens_per_sequence: List[int]) -> InPlaceCacheData:
        ...
        for sequence_id, num_tokens in zip(sequence_ids, num_tokens_per_sequence):
            ...
            max_sequence_length = max(max_sequence_length, self.context_map[sequence_id])
            context_lengths.append(self.context_map[sequence_id])

        for i in range(self.num_layers):
            empty_tensor_k = torch.empty(
                size=(len(num_tokens_per_sequence), self.num_heads, max(num_tokens_per_sequence), self.head_size),
                dtype=self.dtype,
                device=self.device
            )
            empty_tensor_v = torch.empty(
        ...
        return InPlaceCacheData(data=self.cache, context_lengths=context_lengths, max_sequence_length=max_sequence_length, sequence_ids=sequence_ids, is_generating=True)
