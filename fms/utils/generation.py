...
def generate(
    ...
    #         wait=10, warmup=2, active=2, repeat=1
    #     ),
    #     on_trace_ready=functools.partial(trace_handler, output_path="/lustre/aviros/mixtral_traces", extra_name=str(rank)),
    #     with_stack=True,
    #     profile_memory=True,
    #     record_shapes=True,
    # ) as prof:
    ...
    print(f"First token latency: {token_times[0]}, average after: {statistics.fmean(token_times[1:])}")
    if not batched:
        result = result[0]
    return result
