...
class _Stateful_Dataset(data.IterableDataset):
    ...
    def __init__(
        ...
        self.load_worldsize = worldsize  # Enable calling load_state_dict() directly, assume no rescaling
...
class Preload_Buffer_Dataset(_Wrapper_Dataset):
    ...
    def __init__(self, dataset: _Stateful_Dataset, window_size: int):
        super().__init__(dataset)
        assert window_size > 1, f"Window size {f} must be greater than 1 for shuffling to occur"
    ...
    def __iter__(self):
        ...
        while True:
            ...
            prefix_length = torch.randint(
                self.prefix_split_pos, len(out1) - (self.prefix_split_pos - 1), (1,), generator=self.generator
            ).item()

            # construct encoder and decoder inputs
            postfix_length = len(out1) - prefix_length
            encoder_input = out1[:prefix_length] + [self.pad] + out2[:postfix_length] + [self.pad]
            decoder_input = [self.bos] + out1[prefix_length:] + [self.pad] + out2[postfix_length:]
            ...
            target = out1[prefix_length:] + [self.ignore_index] + out2[postfix_length:] + [self.ignore_index]
...
class Streaming_Doc_Dataset(_Stateful_Dataset):
    ...
    def __init__(
        ...
        for dataset in self.datasets:
            ...
            shardfrags = [(shards[i // worldsize], i % worldsize) for i in range(start_frag, end_frag)]
            ...
            for i, (shard, frag) in enumerate(shardfrags):
                ...
                # Read into shardset
                shardset += [(dataset, shardcount, i) for i in range(doc_start, doc_end)]
            # Add final shardset
            if len(shardset) > 0:
                docset.append(shardset)
            ...
            if verbose:
                logging.info(f"    Worker {rank} ingested {len(shardfrags)} shard fragments from {dataset}")
        ...
        while True:
            ...
            for i in range(ndocs):
                ...
                self.dataset_percent_seen[dataset] = (
                    self.dataset_docs_seen[dataset] * 100 / (self.docs_per_dataset[dataset] + 1e-9)
                )
...
class Sampling_Dataset(_Stateful_Dataset):
    ...
    def __init__(
        ...
        self.weights = [1 if weights is None else weights[i] for i in range(len(self.datasets))]
        ...
        for i, d in enumerate(self.datasets):
            ...
            if verbose:
                logging.info(f"Worker {rank} assembled subdataset iterator for {d}, {i+1} of {len(self.datasets)}")
    ...
    def load_state_dict(self, state_dicts, sharded_input=False):
        ...
        for i, subdata in enumerate(self.data):
            ...
            subdata.load_state_dict([sd[self.statename("sample_iterator_states")][i] for sd in sharded_dicts], True)
        return sharded_dicts
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if self.current_reader is not None:
                ...
                ind = random.choices(list(range(self.n_logicals)), weights=self.n_docs_remaining, k=1)[0]
...
class Shard_Doc_Dataset(_Stateful_Dataset):
    ...
    def __init__(
        ...
        for dataset in self.datasets:
            ...
            reader = None
            if verbose:
                logging.info(f"    Worker {rank} ingesting {len(shardfrags)} shard fragments")
            for i, (shard, frag) in enumerate(shardfrags):
                if (i + 1) % 100 == 0 and verbose:
                    logging.info(f"    Worker {rank} ingested {i+1} of {len(shardfrags)} shard fragments")
                # Grab new reader if new
                if shard != last_shard:
            ...
            if verbose:
                logging.info(f"    Worker {rank} ingested {len(keylist)} documents from {dataset}")
...
class Shard_Line_Dataset(_Stateful_Dataset):
    ...
    def __init__(
        ...
        self.datasets = (
            datasets
            if datasets is not None
            else [f for f in os.listdir(datapath) if not os.path.isfile(os.path.join(datapath, f))]
        )
        if testrun_data_index != -100:
            assert (testrun_data_index >= 0) and (
                testrun_data_index < len(self.datasets)
        ...
        random.seed(self.seed + rank)  # Guaranteed inconsistent shuffling across workers
        ...
        while True:
            ...
            for i in range(ndocs):
                ...
                for j in range(nlines):
                    if i == 0 and j <= residual_lines:  # If resuming from a doc, ignore the required number of lines
                        pass
                    else:
                        self.line_index = j
