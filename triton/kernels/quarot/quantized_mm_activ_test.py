...
def quantized_mm_activ(x, w, activ):
...
for i in range(0, runs_per_test):
    ...
    activ = lambda x: torch.nn.functional.silu(x, 1)

    # TODO: generate rotation matrices every time
    results_basic.append(quantized_mm_activ(x, w, activ))
    results_rot_rand.append(quantized_mm_activ(x_rot_rand, w_rot_rand, activ))
    results_rot_rand_transp.append(quantized_mm_activ(x_rot_rand, w_rot_rand_transp, activ))
    results_rot_hard.append(quantized_mm_activ(x_rot_hard, w_rot_hard, activ))
