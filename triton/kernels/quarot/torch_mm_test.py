...
for i in range(0, runs_per_test):
    ...
    # results_rot_rand.append(quantize_mm_avg_err(x @ r_rand, r_inv_rand.type(torch.float64) @ w.type(torch.float64)))
    # results_rot_hard.append(quantize_mm_avg_err(x @ r_hard, r_inv_hard.type(torch.float64) @ w.type(torch.float64)))
