...
def test_gptoss_equivalence():
    """Tests GPT Oss equivalence with a known implementation. Takes approximately 1:11 on an mbp with M1 chip"""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer, pipeline

    gpt_oss_model_path = "openai/gpt-oss-20b"
    tokenizer = AutoTokenizer.from_pretrained(gpt_oss_model_path, use_fast=True)
    hf_model = AutoModelForCausalLM.from_pretrained(gpt_oss_model_path)

    # convert the hf model to fms
    model = get_model("hf_pretrained", "openai/gpt-oss-20b")
    print(model)
